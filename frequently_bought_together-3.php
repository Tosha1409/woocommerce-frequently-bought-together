...
{
	if ((is_countable($products)) && (count($products)<4)){return;}

	#added meta_query to avoid private products and products of of stock  
...
#getting amount of products for forming full rows
    $max_products = $products_list->post_count;
    if (($products_list->post_count)>3){
    $max_products = ($products_list->post_count) - (($products_list->post_count) % 4);}
    $product_counter=0;
	...
	if ( $products_list->have_posts() ) : ?>
		...
		<div class="related products">
			...
			<?php woocommerce_product_loop_start(); ?>
				...
				<?php while ( ( $products_list->have_posts() ) && ($product_counter != $max_products) ) : $products_list->the_post(); $product_counter++; ?>
	...
	if($products_buy){
		...
		if ( $products_buy->have_posts() ){
			while ( $products_buy->have_posts() ){
				...
				$cart_content = '';
				$cart_content .= '<div class="bought_prd product" price="'.$prd_price.'">';
		...
		if($add_to_cart_arr){
			...
			echo '<h2>'.__('Frequently Bought Together','aheadzen').'</h2>';
			echo '<div id="bought_together_frm">';
			echo '<div class="bought_together_prds products">';
			echo implode('',$add_to_cart_arr);
			...
			echo '<script>
				jQuery("#bought_together_frm input:checkbox").click(function() {
					...
					jQuery(".also_bought_css_button").html(button_text);
					
				});
				
				jQuery(".also_bought_css_button").click(function() {
					...
					if(priceval){
						...
						jQuery("a.also_bought_css_button").attr("href",addtocarturl);
					}else{
						return false;
					}
				});
